            ...
            try {
              ...
              if (data.type === 'message' && data.chatJid && data.text) {
                ...
                if (isAuthorized(data.chatJid)) {
                  ...
                  } else if (/\/workspace\/[^/]+\/group\//.test(imagePath)) {
                    // Lume VM path: /Users/lume/workspace/{safeName}/group/... -> GROUPS_DIR/sourceGroup/...
                    const m = imagePath.match(/\/workspace\/[^/]+\/group\//)!;
                    const relPath = imagePath.slice(m.index! + m[0].length);
